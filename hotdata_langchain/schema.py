...
def _table_indexes(
    ...
    *,
    ...
    """Return every search index on one table, ready or not, or none when unscoped.
    ...
    Unready indexes are kept because the caller makes two different decisions from this
    list, and only one of them turns on readiness. Whether a column is searchable *yet*
    does; whether a column was generated by an index does not. Filtering here would name
    no capability and also stop hiding the vector column, which is the worse of the two
    failures — it puts a float list in front of the model as ordinary data.
    """
    if database is None:
        logger.debug("no database scope for %s.%s; not reporting searchable columns", schema, name)
        return []
    return list_search_indexes(
        client, table=name, schema=schema, database=database, ready_only=False
    )
...
def describe_tables_json(
    ...
    *,
    ...
    # Generated-ness does not depend on the index having finished building; being
    # searchable does. Same listing, two filters.
    ...
    searchable = search_nouns_by_column([index for index in indexes if index.ready])
    annotated = False
